...
module ssram (
	...
	input	[7:0]	burst_wdata,		//	Write data (clk_129m domain)
	input			burst_wdata_en,		//	Write data valid (clk_129m domain)
	...
	// ---------------------------------------------------------
	reg				ff_burst_mode;			// Burst write active (clk_129m domain)
	reg	[16:0]		ff_burst_count;			// Remaining bytes to write
	reg				ff_burst_active;		// Burst active flag (clk_129m domain)
	...
	// CDC: burst_start (clk -> clk_129m)
	reg	[2:0]		ff_burst_start_sync;
	wire			w_burst_start_pulse;

	always @( negedge clk_129m ) begin
		if( !reset_n ) begin
	...
	always @( negedge clk_129m ) begin
		...
		else if( ff_state == c_state_idle && ff_burst_start_req ) begin
			ff_burst_start_req <= 1'b0;
		end
	end
	...
	// Burst FIFO write logic (every clk_129m cycle)
	always @( negedge clk_129m ) begin
	...
	always @( negedge clk_129m ) begin
		...
		else if( ff_state == c_state_burst_write1 ) begin
	...
	assign sram_sclk	= clk_129m & ~ff_ce_n & ~w_burst_pause;
